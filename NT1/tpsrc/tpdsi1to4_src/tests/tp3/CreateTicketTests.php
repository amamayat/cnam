...
class CreateTicketTests extends AbstractCommonWebTest {
	...
	function setUp() {
		...
		if ( file_exists($this->persistantFilePath) )
	...
	function notatestgetProjectRootUrl() {
		$createTicketURL = $this->getProjectRootUrl() . 'tp3/editTicket.php';
		$expectedCreateTicketURL = 'http://localhost:80/tpdsi/tp3/editTicket.php';
	...
	function createAppNameFile($file) {
		$f = fopen($this->appNamesFilePath, 'w');
		$apps = <<<EOT
1;    Paye
2;Inscriptions
3;  Achats
